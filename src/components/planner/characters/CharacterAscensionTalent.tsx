...
function CharacterAscensionTalent({ character }: { character: CharacterCostObject }) {
    ...
    React.useEffect(() => {
        dispatch(updateCharacterCosts({ name: name, type: "talent", costs: GetCost(sliderValue) }))
    ...
    return (
        ...
        >
            <Box sx={{ display: "flex", alignItems: "center" }}>
                ...
                <CardHeader
                    avatar={<img alt={name} src={(`${process.env.REACT_APP_URL}/characters/skills/${name.split(" ").join("_").toLowerCase()}_talent.png`)} style={{ width: "48px", height: "48px", border: `1px solid ${theme.border.color}`, borderRadius: "48px" }} onError={ErrorLoadingImage} />}
