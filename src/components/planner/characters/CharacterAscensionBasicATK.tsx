...
function CharacterAscensionBasicATK({ character }: { character: CharacterCostObject }) {
    ...
    let materialCosts = characterSkillLevel[rarity.toString() as keyof CharacterSkill].attack
    ...
    React.useEffect(() => {
        dispatch(updateCharacterCosts({ name: name, type: "attack", costs: GetCost(sliderValue) }))
    ...
    return (
        ...
        >
            <Box sx={{ display: "flex", alignItems: "center" }}>
                ...
                <CardHeader
                    avatar={
                        <img alt={name} src={(`${process.env.REACT_APP_URL}/characters/skills/${name.split(" ").join("_").toLowerCase()}_attack.png`)} style={{ width: "48px", height: "48px", border: `1px solid ${theme.border.color}`, borderRadius: "48px" }} onError={ErrorLoadingImage} />
                    }
                    title={
            ...
            <Box sx={{ display: "flex", alignItems: "center", px: 2 }}>
                ...
                <CustomSlider disabled={!selected} value={sliderValue} step={1} min={1} max={maxValue} onChange={handleSliderChange} disableSwap />
